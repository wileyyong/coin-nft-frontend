...
import { SmallTextTitleGrey } from "components/common/common.styles";
...
import OwnedItem from "components/myitems/OwnedItem";
...
const CollectionDetail: React.FC<CollectionDetailProps> = () => {
  ...
  const getUserImgAvatar = () => {
    if (collectionInfo.creator && collectionInfo.creator.avatar)
      return `${configs.DEPLOY_URL}${collectionInfo.creator.avatar}`;
    return imgAvatar;
  };
  const getCollectionInfoImage = () => {
    if (collectionInfo.image) {
      if (collectionInfo.image.includes("https://"))
  ...
  return (
    <Layout className="collection-detail-container">
      <div className="d-flex flex-column align-items-center">
        <div
          style={{ backgroundImage: `url("${getCollectionInfoImage()}")` }}
          className="background-item"
        ></div>
        <div className="d-flex flex-row align-items-center justify-content-center">
          <div
            className="avatar"
            style={{ backgroundImage: `url(${getUserImgAvatar()})` }}
          ></div>
        </div>
      </div>
              ...
              {loading && pageNum === 1 ? (
                ...
                <div className="auction-section">
                  <div className="item-list mt-3 mt-md-5">
                    {itemList.length > 0 ? (
                      itemList.map((item, i) => (
                        <OwnedItem key={i} item={item}></OwnedItem>
                      ))
                    ) : (
                      <NoItem
                        title="No items found"
