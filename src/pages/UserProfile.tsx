...
const UserProfile: React.FC<UserProfileProps> = () => {
  ...
  useEffect(() => {
    const loadTokensAndCollections = async () => {
      ...
      if (walletAddress) {
        ...
        let { collections } = await CollectionController.getByUserId(
          data.user._id
        );
  ...
  useEffect(() => {
    if (selectedFilter) {
      setFilteredActivities(
        itemList.filter(
          (token: { type: String }) => token.type === selectedFilter
        )
      );
    } else {
  ...
  const onSelectFilter = (filter: String) => {
    ...
    if (selectedFilter === filter || filter === "") {
      ...
      setFilteredActivities(
        itemList.filter((token: { type: String }) => token.type === filter)
      );
    }
    setLoading(false);
  };
    ...
    <Layout className="userprofile-container">
      <div className="d-flex flex-column align-items-center">
        <div
          style={{
            backgroundImage: `url(${
              userInfo.cover ? configs.DEPLOY_URL + userInfo.cover : CoverImage
            })`
          }}
        ...
        <div className="display user-name pb-3">
