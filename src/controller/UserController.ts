...
class UserController {
  public static signInWithWallet(payload: any) {
    return API.post(configs.API.AUTH_WITH_WALLET_URL, payload).then(
      (res) => res.data
    );
  }
  ...
  public static getSettings() {
    return API.get(`${configs.API.USERS_URL}/settings`).then(
      (res) => res.data.user
    );
  }
  ...
  public static getTopUsers(type: string, days: number) {
    return API.get(`${configs.API.USERS_URL}/tops/${type}?days=${days}`).then(
      (res) => res.data.users
    );
  }
  ...
  public static userSearch(params: any, page: number) {
    return API.post(
      `/users/search${page === 1 ? "" : "?page=" + page}`,
      params
    ).then((res) => res.data);
  }
  public static qrConnect(payload: any) {
    return API.post(`${configs.API.USERS_URL}/qrconnect`, payload).then(
      (res) => res.data
    );
  }
  ...
  public static getDailySteps(payload: any) {
    return mobileAPI
      .post(`${configs.API.PUMLUSER_URL}/getDailySteps`, payload)
      .then((res) => res.data);
  }
  public static pumlxApproved(id: string) {
    return API.get(`${configs.API.USERS_URL}/${id}/pumlxApproved`);
  }
}
